...
AI.init = function(pace) {
	...
	if (val && val.value != -8888) {
		...
		com.get("moveInfo").innerHTML = '<h3>AI搜索结果：</h3>最佳着法：' + com.createMove(com.arr2Clone(play.map), man.x, man.y, val.x, val.y) + '<br />搜索深度：' + AI.treeDepth + '<br />搜索分支：' + AI.number + '个 <br />最佳着法评估：' + val.value + '分' + ' <br />搜索用时：' + (nowTime - initTime) + '毫秒'
...
AI.getAlphaBeta = function(A, B, depth, map, my) {
	...
	　 //这里排序以后会增加效率
	for (var i = 0; i < moves.length; i++) {　　 //走这个走法;
		var move = moves[i];
		var key = move[4];
		var oldX = move[0];
		var oldY = move[1];
